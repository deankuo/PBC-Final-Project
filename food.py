...
class Food(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.food_boom = pygame.image.load(
            os.path.join("image", "food_boom.png")).convert_alpha()
        self.food_clock = pygame.image.load(
            os.path.join("image", "food_clock.png")).convert_alpha()
        self.food_gun = pygame.image.load(
            os.path.join("image", "food_gun.png")).convert_alpha()
        self.food_iron = pygame.image.load(
            os.path.join("image", "food_iron.png")).convert_alpha()
        self.food_protect = pygame.image.load(
            os.path.join("image", "food_protect.png")).convert_alpha()
        self.food_tank = pygame.image.load(
            os.path.join("image", "food_tank.png")).convert_alpha()
        self.kind = random.choice([1, 2, 3, 4, 5, 6])
        if self.kind == 1:
